...
`npm run test:browser` runs Playwright smoke tests against the local app; set `BASE_URL` to target an existing deployment.
...
- Projects scroll-linked transforms should track scroll directly rather than adding spring lag; `BuildingPreview` decorative motion is disabled on mobile and reduced-motion devices, and mobile spacing/shadows should stay restrained.
- The Beyond the Code gallery must preserve dialog semantics, focus return/trapping, Escape-to-close, visibility-aware preview rotation, and descriptive image alt text.
